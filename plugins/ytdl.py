...
ongoing_downloads: Dict[int, Dict[str, Any]] = {}

# 资源管理
import weakref
import gc
from config import MAX_CONCURRENT_DOWNLOADS, MAX_FILE_SIZE_MB, DOWNLOAD_TIMEOUT, ENABLE_CACHING
...
def d_thumbnail(thumbnail_url: str, save_path: str) -> Optional[str]:
    """下载缩略图（带重试机制）"""
    max_retries = 3
    retry_delay = 1
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.debug(f"缩略图下载成功: {save_path}")
            return save_path
        except requests.exceptions.RequestException as e:
            logger.warning(f"下载缩略图失败 (尝试 {attempt + 1}/{max_retries}): {e}")
            if attempt < max_retries - 1:
                time.sleep(retry_delay)
                retry_delay *= 2
            else:
                logger.error(f"下载缩略图失败，已达到最大重试次数: {e}")
                return None
...
async def process_audio(client, event, url, cookies_env_var=None):
    ...
    progress_message = await event.reply("**__开始音频提取…__**")
    ...
    try:
        ...
        await progress_message.edit("**__正在写入元数据…__**")
        ...
        if os.path.exists(download_path):
            ...
            prog = await client.send_message(chat_id, "**__开始上传…__**")
            ...
            await event.reply("**__提取后未找到音频文件！__**")
 
    except Exception as e:
        logger.exception("Error during audio extraction or upload")
        await event.reply(f"**__发生错误：{e}__**")
    finally:
        if os.path.exists(download_path):
...
async def handler(event):
    ...
    # 检查下载限制
    if len(ongoing_downloads) >= MAX_DOWNLOADS:
        await event.reply(f"**当前下载队列已满 ({len(ongoing_downloads)}/{MAX_DOWNLOADS})，请稍后再试！**")
        return
    
    if user_id in ongoing_downloads:
        await event.reply("**你已有一个下载在进行，请等待完成后再试！**")
        return 
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        if check_duration_and_size:
            ...
            if duration and duration > 3 * 3600:   
                await progress_message.edit("**❌ __视频长度超过 3 小时，已取消下载…__**")
                return None
 
             
            estimated_size = info_dict.get('filesize_approx', 0)
            if estimated_size and estimated_size > 2 * 1024 * 1024 * 1024:   
                await progress_message.edit("**🤞 __视频大小超过 2GB，已取消下载。__**")
                return None
...
async def handler(event):
    ...
    if user_id in ongoing_downloads:
        await event.reply("**你已有一个 ytdlp 下载在进行，请等待完成！**")
        return 

    if len(event.message.text.split()) < 2:
        await event.reply("**用法：**`/dl <视频链接>`\n\n请提供有效的视频链接！")
        return    
...
async def process_video(client, event, url, cookies_env_var, check_duration_and_size=False):
    ...
    try:
        ...
        if os.path.exists(download_path):
            ...
            await event.reply("**__下载完成后未找到文件，可能出现问题！__**")
    except Exception as e:
